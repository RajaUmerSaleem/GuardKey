...
export default function App() {
  ...
  const handleAdd = () => {
    ...
    if (input.password === '') {
      toast("Empty Password is Not Allowed!");
    }
        ...
        <div className=' w-full sm:w-[70%] h-[62%] overflow-y-scroll mx-auto'>
          <table className='h-full w-full  bg-white border border-gray-500'>
            ...
            {dataArray.map((item, index) => (
              <tr key={index} className='h-[12%] w-full   flex hover:bg-gray-200 cursor-pointer'>
                <td className="w-[42%] sm:w-[45%]  border border-gray-500 sm:px-2 text-left flex-wrap">{item.url}</td>
                <td className="w-[20%]  border border-gray-500 sm:px-2 text-wrap">{item.username}</td>
                <td className="w-[20%]  border border-gray-500 sm:px-2 text-left flex flex-col gap-0"> {item.Mode ? item.password : "******"}
                  <img
